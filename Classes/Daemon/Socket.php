...
abstract class Socket
{
    ...
    public function __construct($bind_address = 0, $bind_port = 0, $domain = AF_INET, $type = SOCK_STREAM, $protocol = SOL_TCP)
    {
        ...
        {
            throw new socketException("Could not bind socket to [$bind_address - $bind_port]: " . socket_strerror(socket_last_error
                ($this->socket))) ;
        }
    ...
    public function write($buffer, $length = 4096)
    {
        ...
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource 2") ;
        }
    ...
    public function read($length = 4096)
    {
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource 3") ;
        } 
    ...
    public function connect($remote_address, $remote_port)
    {
        ...
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource 4") ;
            return false ;
        } 
        elseif (@socket_connect($this->socket, $remote_address, $remote_port) == false)        
        {
            $error = socket_last_error($this->socket) ;
            if ($error == S_INPROGRESS)
            {
                return true ;
            }
            throw new socketException("Could not connect to {$remote_address} - {$remote_port}: [".$error."]" . $this->get_error()) ;
            return false ;
        }
        return true ;
    }
    ...
    public function listen($backlog = 128)
    {
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource 5") ;
        } 
    ...
    public function accept()
    {
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource 6") ;
        } 
    ...
    public function set_non_block()
    {
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource7") ;
    ...
    public function set_block()
    {
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource 8") ;
    ...
    public function set_recieve_timeout($sec, $usec)
    {
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource 9") ;
    ...
    public function set_reuse_address($reuse = true)
    {
        ...
        if (!is_resource($this->socket))
        {
            throw new socketException("Invalid socket or resource 10") ;
